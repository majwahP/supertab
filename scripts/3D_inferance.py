...
from supertrab.inferance_utils import load_3d_model, generate_sr_images_3D
...
def main(
    ...
    model = load_3d_model(weights_path, image_size=patch_size[-1], device=device)
    model.eval()
    ...
    for batch in tqdm(dataloader, desc="Patches"):
        ...
        sr_images = generate_sr_images_3D(
            model,
            scheduler,
            lr_images,
        ...
        for sr_patch, position in zip(sr_images, batch["position"]):
            ...
            dz, dy, dx = patch_size
            sr_np = sr_patch.cpu().numpy().astype(np.float32).squeeze(0)
...
if __name__ == "__main__":
    ...
    main(
        ...
        weights_path=f"samples/supertrab-diffusion-sr-3d/{PATCH_SIZE}_ds{DS_FACTOR}/models/final_model_weights_{PATCH_SIZE}_ds{DS_FACTOR}.pth",
        patch_size=(PATCH_SIZE, PATCH_SIZE, PATCH_SIZE),
        downsample_factor=DS_FACTOR,
        batch_size=1, 
        sr_dataset_name=f"sr_volume_{PATCH_SIZE}_{DS_FACTOR}_3D"
    )
