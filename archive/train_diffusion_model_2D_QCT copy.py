...
def main():
    ...
    gt_train_dataloader = create_dataloader(zarr_path, downsample_factor=config.ds_factor, patch_size=(1, config.image_size, config.image_size), groups_to_use=train_groups, data_dim="2d", with_blur=False, draw_same_chunk=True, shuffle=False)
    train_dataloader = create_dataloader(zarr_path, downsample_factor=config.ds_factor, patch_size=(1, config.image_size, config.image_size), groups_to_use=train_groups, data_dim="2d", with_blur=False, image_group="registered_LR_upscaled_trimmed", draw_same_chunk=True, shuffle=False)
    gt_val_dataloader = create_dataloader(zarr_path, downsample_factor=config.ds_factor, patch_size=(1, config.image_size, config.image_size), groups_to_use=val_groups, data_dim="2d", with_blur=False, draw_same_chunk=True, shuffle=False)
    val_dataloader = create_dataloader(zarr_path, downsample_factor=config.ds_factor, patch_size=(1, config.image_size, config.image_size), groups_to_use=val_groups, data_dim="2d", with_blur=False, image_group="registered_LR_upscaled_trimmed", draw_same_chunk=True, shuffle=False)
    gt_test_dataloader = create_dataloader(zarr_path, downsample_factor=config.ds_factor, patch_size=(1, config.image_size, config.image_size), groups_to_use=test_groups, data_dim="2d", with_blur=False, draw_same_chunk=True, shuffle=False)
    test_dataloader = create_dataloader(zarr_path, downsample_factor=config.ds_factor, patch_size=(1,config.image_size,config.image_size), groups_to_use=test_groups, data_dim="2d", with_blur=False, image_group="registered_LR_upscaled_trimmed", draw_same_chunk=True, shuffle=False)
